...
**forge-kit** is an AI-assisted project governance scaffold: AI-agnostic at the governance layer (issue templates, labels, GWT scenarios), Claude Code-native at the automation layer (agents, skills, slash commands). It is a template repository, not a buildable application. Its purpose is to be bootstrapped into other projects or used as an upgrade reference via the `forge-adapt` skill. There are no build steps or package managers. The only CI is a governance `Validate` workflow (`.github/workflows/validate.yml`): it runs the structural check, the template lockstep, seven contract test suites, the component-index freshness check, the component size budget, and (on `pull_request` only) the two range guards, plus one advisory `claude plugin validate` step marked `continue-on-error` that can report issues without failing the build. There is no application build/test pipeline. Four of those seven suites cover shipped executables (hooks, the catalogue script, `forge-lib.sh`, the component-index generator); the other three cover the repo's own guards (`test-template-lockstep.sh`, `test-check-plugin-version-bump.sh`, `test-component-size.sh`), and all of those run unconditionally even though one of the guards they cover is PR-only.
...
| `forge-kit-governance` | 0.7.12 | agent: ticket-gate; command: gate-ticket; skills: closing-sessions, working-overnight; hooks: block-dashes, overnight-continue, overnight-guard |
...
**The enforced path set is a fixed glob, duplicated in all three guards.** A file is marker-enforced only if it matches `plugins/*/agents/*.md`, `plugins/*/commands/*.md`, `plugins/*/skills/*/SKILL.md`, `plugins/*/hooks/*.{py,sh}`, or `plugins/*/skills/*/assets/*.sh`. Everything else under `plugins/` is unversioned and unguarded by construction: `references/*` (deliberate, they are supporting prose), but also `skills/*/scripts/*.py` and non-`.sh` assets, which is why `closing-sessions/scripts/memory.py` and `api-design-principles/assets/rest-api-template.py` carry no marker today. Widening the set means editing the same regex in `scripts/validate-plugins.sh` (as a `find` expression) and in both `scripts/check-version-bump.sh` and `.githooks/pre-commit` (as a `grep -E` alternation).
